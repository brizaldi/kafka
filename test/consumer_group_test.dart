...
import 'setup.dart';
import 'consumer_group_test.mocks.dart';

@GenerateMocks([KafkaSession])
void main() {
  group('ConsumerGroup:', () {
    late MockKafkaSession _session;
    String _topicName = 'dartKafkaTest';
    late Broker _coordinator;
    late Broker _badCoordinator;
    ...
    test('it tries to refresh coordinator host 3 times on fetchOffsets',
      ...
      when(_session.getConsumerMetadata('testGroup')).thenReturn(
        Future.value(GroupCoordinatorResponse(
          0,
          _badCoordinator.id,
          _badCoordinator.host,
          _badCoordinator.port,
        )),
      );
    ...
    test(
      ...
      when(_session.send(
        argThat(isA<Broker>()),
        argThat(isA<OffsetFetchRequest>()),
      )).thenReturn(Future.value(OffsetFetchResponse.fromOffsets(badOffsets)));
